...
import { useProfileEditMutations } from "./hooks/useProfileEditMutations";
...
export function ProfileClient({
  ...
  initialDashboard,
  subscriptionUsage,
}: ProfileClientProps): JSX.Element {
  const router = useRouter();
  const isMobile = useMediaQuery("(max-width: 768px)");

  const [profile, setProfile] = useState<UserProfile>(mapApiProfileToUserProfile(initialProfile));
  ...
  const { editProfileMutation, avatarMutation } = useProfileEditMutations({
    initialProfile,
    setProfile,
    setError,
  });
  ...
  const handleRenew = (): void => router.push("/pricing");
  const handleUpgrade = (): void => setIsSlideOverOpen(true);
  const handleViewTemplate = (slug: string): void => router.push(`/p/${slug}`);
  const handleDeleteTemplate = async (_slug: string): Promise<void> => {
    // Not yet wired to a server action.
  };
  ...
  // `error` is surfaced inline by each mutating card; nothing global to render here.
  void error;
